...
export function LoginPage() {
    ...
    useEffect(() => {
        if (admin) {
            if (admin.role == ROLE.admin) redirectToAdminPage()
        }
    }, [admin])
...
export function FormLogin() {
    const { loginAdminrWithEmailAndPassword } = useAuth()
    const alert = useAlert()

    const login = async (data) => {
        await loginAdminrWithEmailAndPassword(data.email, data.password).catch((error) => {
            console.log(error.response)
            alert.error('Lỗi đăng nhập. ' + error.response.data.error.message)
        })
    }
    return (
        <Form
