...
const HeaderButton = ({
    ...
    const { refreshing, itemName, onRefresh, loadAll } = useDataTable()
    const ref = useRef()
...
const HeaderButtons = ({ children, className }: ReactProps) => (
    <div className={`flex gap-x-2 ${className}`}>{children}</div>
)
